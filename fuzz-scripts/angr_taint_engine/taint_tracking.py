...
def is_or_points_to_tainted_data(x, state):
    return is_tainted(x, state) or \
           is_tainted(state.memory.load(x, 32), state)
            ...
            try:
                ...
                for st in simgr.active:
                    ...
                    if st.addr not in self.project.kb.functions and not has_cfg:
                        ...
                        # cfg = self.project.analyses.CFGEmulated(starts=[0x010590], call_depth=2)
                        plot_cfg(cfg, "/home/jackfromeast/bamboofuzz/fuzz-scripts/test_arm_i", asminst=True, remove_imports=True, remove_path_terminator=True)
                        has_cfg = True
