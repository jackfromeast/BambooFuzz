...
class Xpath_Util:
    ...
    def __init__(self, driver):
        ...
        self.known_attribute_list = ['id','name','placeholder','value','title','type','class'] #已知的元素属性列表
        # self.variable_names = []
                        ...
                        if element.has_attr(attr):#这里对一些常见属性进行判断
                            ...
                            try:
                                if len(self.driver.find_elements_by_xpath(locator))==1:
                                    result_flag = True
                                    if element['type'] == 'button':#把button的字典存在一个字典里面去
    ...
    def guess_xpath_other(self,element):
        ...
        if(flag==False):
            ...
            # print("We are not supporting this extra gussable element") 
...
if __name__ == "__main__":
    ...
    if xpath_obj.generate_xpath(soup) is False:
        print ("[Debug] No XPaths generated for the URL:%s"%url)
